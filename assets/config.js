 ...
 */
window.ASTRA_CONFIG = {
  // ▼管理画面（admin.html）のログインパスワード設定
  // パスワードそのものではなく、パスワードを暗号化（ハッシュ化）した文字列を保存しています。
  // これにより、このファイルを誰かに見られても、元のパスワードはわかりません。
  //
  // ▼パスワードを変更したい場合
  // 新しいパスワードをClaudeに伝えて「ハッシュ化して」とお願いすれば、
  ...
  SHORTS_FILTER: {
    // この秒数以下の動画はショートとみなして除外します(1分15秒 = 75秒)
  ...
  // トップページ（index.html）の、イントロ文のすぐ下・LIVE配信セクションの直前に表示する、
  // 記事下の関連リンクと同じ「テキストのみ」スタイルの設定。
  // パソコン・スマホどちらでも必ず表示されるメインエリアの中に置くための枠。
  AFFILIATE_INTRO_LINK: {
    enabled: true,
    heading: 'おすすめ',
  },
  // ▲ここまで追加 ============================================
